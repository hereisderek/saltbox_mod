...
rsync -avhP /mnt/local/Media/ /mnt/remote/media/Media/ | tee -a "$log_file"
printf "\n" | tee -a "$log_file"
...
duration_h="%02d:%02d:%02d\n" $((duration/3600)) $(( (duration%3600)/60 )) $((duration%60))
printf "total duration: ${duration_h}\n" | tee -a "$log_file"

#curl -X POST --data-binary @${log_file} "$ping_url/log"; curl -fsS -m 10 --retry 5 -o /dev/null "$ping_url"
